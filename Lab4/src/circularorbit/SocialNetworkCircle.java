...
public class SocialNetworkCircle extends CircularOrbit<CentralUser, Friend> {
  ...
  public static SocialNetworkCircle createInstanceFromClient(String fileName)
      throws FileNotFoundException, SameLabelException, DependencyException,
      GrammarException {
    ...
    for (int i = 0; i < froms.size(); i++) {
      ...
      if (relations.contains(relation1) || relations.contains(relation2)) {
        throw new DependencyException();
      }
    ...
    checkRep(SocialNetworkCircle.builder().c2tRelations(c2tRelations).t2tRelations(t2tRelations)
   ...
   *
   * @param sc the SocialNetworkCircle will check
   */
