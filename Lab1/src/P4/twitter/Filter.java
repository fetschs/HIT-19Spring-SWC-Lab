...
public class Filter {
    ...
    public static List<Tweet> writtenBy(List<Tweet> tweets, String username) {
        ...
        for (Tweet tweet : tweets) {
            String twweetAuthor = tweet.getAuthor();
            if (twweetAuthor.toLowerCase().equals(factusername)) {
                retList.add(tweet);
            }
        }
        return retList;
    ...
    public static List<Tweet> inTimespan(List<Tweet> tweets, Timespan timespan) {
        ...
        for (Tweet tweet : tweets) {
            Instant tweettimespan = tweet.getTimestamp();
            if (start.isBefore(tweettimespan) && end.isAfter(tweettimespan)) {
                retList.add(tweet);
            }
        }
        return retList;
